...
newtype Angle n = Radians (V1 n)
  deriving (Read, Show, Eq, Ord, Functor, Additive, Metric)
...
rad :: Iso' (Angle n) n
rad = iso (\(Radians (V1 r)) -> r) (Radians . V1)
...
turn = iso (\(Radians (V1 r)) -> r / (2*pi)) (Radians . V1 . (*(2*pi)))
...
deg = iso (\(Radians (V1 r)) -> r / (2*pi/360)) (Radians . V1 . ( * (2*pi/360)))
