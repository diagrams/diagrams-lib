...
--  first quadrant.
bezierFromSweepQ1 :: (R2Ish v) => Angle (Scalar v) -> Segment Closed v
...
bezierFromSweep :: (R2Ish v) => Angle (Scalar v) -> [Segment Closed v]
bezierFromSweep s
...
arcT :: (R2Ish v) => Direction v -> Angle (Scalar v) -> Trail v
...
--   @Trail@ is allowed to wrap around and overlap itself.
arc :: (R2Ish v, TrailLike t, V t ~ v) => Direction v -> Angle (Scalar v) -> t
...
--   >        # centerXY # pad 1.1
arc' :: (R2Ish v, TrailLike p, V p ~ v) => Scalar v -> Direction v -> Angle (Scalar v) -> p
...
--   >   # fc blue
--   >   # centerXY # pad 1.1
wedge :: (R2Ish v, TrailLike p, V p ~ v) => Scalar v -> Direction v -> Angle (Scalar v) -> p
...
--   >   # centerXY # pad 1.1
arcBetween :: (R2Ish v, TrailLike t, V t ~ v) => Point v -> Point v -> Scalar v -> t
...
--   >   # fc blue
--   >   # centerXY # pad 1.1
annularWedge :: (R2Ish v, TrailLike p, V p ~ v) =>
