...
import Diagrams.TwoD.Util (unitX, unitY, unit_Y)
...
--   @[P2]@ could be generated by a call to 'polygon', since a list of
--   vertices is 'PathLike'.  But of course the list can be generated
--   any way you like.  A @'Path' 'R2'@ is returned (instead of any
--   'PathLike') because the resulting path may have more than one
--   component, for example if the vertices are to be connected in
--   several disjoint cycles.
starPoly :: StarOpts -> [P2] -> Path R2
starPoly sOpts vs = graphToPath $ mkGraph f vs
